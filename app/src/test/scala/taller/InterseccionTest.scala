...
class InterseccionTest extends AnyFunSuite {
  ...
  test("Interseccion con argumento de 1 devuelve el mínimo") {
    ...
    //cd1 y cd2 de la funcion Interseccion
    val cd1: Int => Double = n => if (n == 1) 0.7 else 0.2
    val cd2: Int => Double = n => if (n == 1) 0.5 else 0.9

    val interseccion = objInter.Interseccion(cd1, cd2)
    //Se asigna un argumento a la funcion, en este caso 1
    assert(interseccion(1) === 0.5)

  }

  test("Interseccion con argumento de 2 devuelve el mínimo") {
    ...
    val interseccion = objInter.Interseccion(cd1, cd2)
    //Se asigna un argumento a la funcion, en este caso 1
    assert(interseccion(2) === 0.2)

  }

  test("Comparacion de ambos conjuntos") {
    val cd1: Int => Double = n => if (n == 1) 0.3 else 0.8
    val cd2: Int => Double = n => if (n == 1) 0.7 else 0.8
  ...
  test("Interseccion que tiene al menos un valor igual a 0.5") {
    val cd1: Int => Double = n => if (n == 2) 0.5 else 0.8
    val cd2: Int => Double = n => if (n == 2) 0.7 else 0.3

    val interseccion = objInter.Interseccion(cd1, cd2)
    val argumentos = List(1, 2, 3)//Nuestros valores n para evaluar
    //Cada elemento de la lista se almacena en n y despues se evalua en interseccion
    //Si alguno de los resultados es igual a 0.5
    //Entonces el test pasa
    assert(argumentos.exists(n => interseccion(n) == 0.5))
  }

  test("Interseccion con argumentos específicos") {
    val cd1: Int => Double = _ => 0.4
    val cd2: Int => Double = _ => 0.7
    //Los argumentos en este caso no importan
    //Porque las funciones anonimas retornan un valor pre establecido

    val interseccion = objInter.Interseccion(cd1, cd2)
    assert(interseccion(5) === 0.4)
  }
